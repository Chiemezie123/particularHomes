...
export default function LandingNavBar(props: landingNavBarProps) {
  ...
  return (
    <div className="flex max-w-[1278px] items-center justify-between 2xl:gap-[410px]">
      <div>
        <Image src={logo} alt="logo" width={logoWidth} height={logoHeight} />
      </div>
      <div className="flex items-center gap-[24px] 2xl:gap-[48px] ">
        <div>
          <Link href={pathOne}>
        ...
        <div>
          <Button
            children={buttonText}
            onClick={onClick}
            className="rounded-[50px] border border-[#F6F6F6] bg-[#008273] text-white"
          />
        </div>
      </div>
    </div>

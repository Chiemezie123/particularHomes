...
interface ActionCardProps {
  ide?: string;
  name?: string;
  onChange: (imageUrl: string | StaticImageData) => void;
  getId: (id: string) => void;
  setImagePic?:(id:string)=> void;
}

export default function ActionCard({ ide, onChange, getId,setImagePic }: ActionCardProps) {
  ...
  const handlePreviewDisplay = () => {
    if (ide &&isImage && setImagePic) {
      setImagePic(ide); 
    }
  };

  return (
    <div className="flex w-[172px] h-[119px] p-1 justify-center items-center gap-1 rounded-lg border border-[#DFDFDF] bg-white" onClick={handlePreviewDisplay}>
      {isImage ? (
